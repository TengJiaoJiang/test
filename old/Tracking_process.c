...
#define   NUMSAM         (3) 
#define   OncePointNum   (20)
#define   TraceConfNum   (10)       //(30)  
#define   TraceTempNum   (40)       //(30)
#define   PI             (3.141592654)
#define   LOSTNUMBER     (8)        //  4
#define   LOSTNUMBERConfirm (16)    //  20
...
uint32_t TRACKING(MmwDemo_DSS_DataPathObj *obj, float CarrierVel, float CarrierAng)
{
    ...
    for (i = 0; i< OncePointNumber; i++)
    {
        ...
        {
            disTempTemp = obj->detObj2D[i].rangeIdx * 0.8138;
        ...
        xTempTemp = -1.0 * (obj->detObj2D[i].x) / 100.0;
        yTempTemp = obj->detObj2D[i].y / 100.0;
		//vTempTemp = obj->detObj2D[i].dopplerIdx * 0.315;
		fprintf(fWrite, "__ x = %f, y = %f , v = %f  ang = %f, dis = %f, fd = %d\n", xTempTemp, yTempTemp, vTempTemp, angTempTemp, disTempTemp, obj->detObj2D[i].dopplerIdx);
	...
	fprintf(fWrite, "** (BSD-bj) = %d\n", VBSDbj);

	fprintf(fWrite, "** (gap scan,lost,updata, last5 updata BSDWARING_obj->TraceState) %d %d %d %d last = %d state =%d\n", ScanGap, BSDWARING_obj->iScanNum, BSDWARING_obj->iLostNum, BSDWARING_obj->iUpdaNum, LastUpdataNum(10, BSDWARING_obj->TraceState), (BSDWARING_obj->TraceState) * 1);
	
    if (LastUpdataNum(10, BSDWARING_obj->TraceState) >= 4) // 5/4
	{
	...
	if (BSDWARING_obj->Waring == 1 && VBSDbj > 0)
	{
		BSDWARING_obj->iLostNum = 0;
	}
	...
	if (VBSDbjvbig0 > 0 && VBSDbjvsml0 > 0) 
	{
		BSDWARING_ccobj->TraceState = BSDWARING_ccobj->TraceState << 1;
		BSDWARING_ccobj->TraceState = BSDWARING_ccobj->TraceState + 1;
    ...
    if (LastUpdataNum(10, BSDWARING_ccobj->TraceState) >= 3) // 5/3
	{
    ...
    if(BSDWARINGFLAGcc == 0 && BSDWARINGFLAGbj == 0)
    {
        BSDWARINGFLAG = 0;
                ...
                for (j = 0; j < OncePointNumberClearn; j++)
                {
                    if (OnceDataArr[j].used == 0)
                    {
						if (TraceTemp[i].TraceFlag * 1 == 0 && InBSDZero(TraceTemp[i]) * 1 == 1) 
						{
							...
							{
								//fprintf(fWrite, "(ID,flag, v,x,y) %d %d %f %f %f  Yv = %f gate = %f \n", TraceTemp[i].ID, TraceTemp[i].TraceFlag * 1, TraceTemp[i].DPLVel, TraceTemp[i].XkArray_xfit[0][0], TraceTemp[i].XkArray_yfit[0][0], fabs(OnceDataArr[j].y - TraceTemp[i].XkArray_yfit[0][0]) / (ScanGap * TimeGap), 25);
								continue;
							}
							else
							{
							...
							{
								//fprintf(fWrite, "(ID,flag, v,x,y) %d %d %f %f %f xv = %f gate = %f \n", TraceTemp[i].ID, TraceTemp[i].TraceFlag * 1, TraceTemp[i].DPLVel, TraceTemp[i].XkArray_xfit[0][0], TraceTemp[i].XkArray_yfit[0][0], fabs(OnceDataArr[j].x - TraceTemp[i].XkArray_xfit[0][0]) / (ScanGap * TimeGap), 11);
                        ...
                        if (TraceTemp[i].TraceFlag == 1 && LastUpdataNum(SCANNUMBER, TraceTemp[i].TraceState) == CONFNUMBER)   // change  20191125
                        {
                            ...
                            else
                            {
                                if (TraceTemp[i].ToBe == 2)
                                {
                                    ...
                                    fprintf(fWrite, "			temp ID = %d tobe confirm ID =%d\n", TraceTemp[i].ID,  PointIDEX + 1);
        ...
        for (i = 0; i < TraceConfNum; i++)
        {
            ...
            if (TraceConf[i].Used == 1)
            {
				...
				if (InBSDZero(TraceConf[i]) * 1 == 1)
				{
					...
					{
						obj->detObj2D[j].rangeIdx = (int)(TraceConf[i].dis * 10 + 0.5);
						//obj->detObj2D[j].x = (TraceConf[i].Xpast[0] + TraceConf[i].Xpast[1] + TraceConf[i].Xpast[2] + TraceConf[i].Xpast[3] + TraceConf[i].Xpast[4]) / 5.0 * 10 + 0.5;
				...
				{
					obj->detObj2D[j].rangeIdx = TraceConf[i].dis * 10 + 0.5;
					//obj->detObj2D[j].x = (TraceConf[i].Xpast[0] + TraceConf[i].Xpast[1] + TraceConf[i].Xpast[2] + TraceConf[i].Xpast[3] + TraceConf[i].Xpast[4]) / 5.0 * 10 + 0.5;
...
extern char XTrendJudegeDW(TwiceDataKalman trace)
{
    ...
    if(trace.Xpast[4] - trace.Xpast[3] > 0)
    {
        for(i = 4; i >= 1; i--)
        {
            if(trace.Xpast[i] - trace.Xpast[i-1] > 0.2)
            {
                num = num + 1;
            }
        }
