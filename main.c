...
{
	int i = 0, j = 0, k = 0;
	FILE *fp = fopen("/home/robot/PycharmProjects/charmlearn/0813Pro/D2COVERonceFangzhen.txt", "r");FILE *fpout = NULL;
	...
	while (fgets(buf, MAX_LINE, fp) != NULL)
	{
		...
		//if (SCANNUM == 106)
		{
			...
			for (i = 0; i < DSPDataInfomation.numDetObj; i++)
			{
				...
				DSPDataInfomation.detObj2D[i].x = LineData[i * 6 + 7] * 10;
				DSPDataInfomation.detObj2D[i].y = LineData[i * 6 + 8] * 10;
			...
			for (i = 0; i < DSPDataInfomation.numDetObj; i++)
			{
				...
				// fprintf(fpout, " %f  ", DSPDataInfomation.detObj2D[i].x / 10.0 - 0.7);
				fprintf(fpout, " %f  ", DSPDataInfomation.detObj2D[i].x / 10.0);
				fprintf(fpout, " %f  ", DSPDataInfomation.detObj2D[i].y / 10.0);
				fprintf(fpout, " %d", DSPDataInfomation.detObj2D[i].peakVal);
